...
assert.match(createOrder, /admin realtime visibility update/, 'fallback notifies admin only after order items are stored');
const adminOrders = fs.readFileSync('src/app/admin/orders/page.tsx','utf8');
assert.match(adminOrders, /table: 'order_items'/, 'admin order list refreshes when order items arrive');
const liveItemsMigration = fs.readFileSync('supabase/migrations/20260813100000_live_admin_order_items.sql','utf8');
assert.match(liveItemsMigration, /alter publication supabase_realtime add table public\.order_items/, 'order items are published to admin realtime');
