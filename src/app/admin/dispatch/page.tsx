...
export default function DispatchPage() {
  ...
  const load = useCallback(async () => {
    ...
    setOrders(ready);
    const { data: riderRows, error: riderError } = await supabase.from('riders').select('id,name,phone').eq('is_active', true).order('name');
    if (riderError) { setError(riderError.message); setLoading(false); return; }
    setRiders((riderRows || []) as Rider[]);
  ...
  function chooseRider(orderId: string, riderId: string) {
    setSelectedRiders(current => ({ ...current, [orderId]: riderId }));
    const rider = riders.find(item => item.id === riderId);
    if (rider) setDrafts(current => ({ ...current, [orderId]: { ...(current[orderId] || emptyDraft), riderName: rider.name, riderPhone: rider.phone } }));
    else setDrafts(current => ({ ...current, [orderId]: { ...(current[orderId] || emptyDraft), riderName: '', riderPhone: '' } }));
  }
  ...
  async function dispatch(order: DispatchOrder) {
    ...
    if (!response.ok) setError(result.error || 'Unable to dispatch this order.');
      ...
      const riderPhone = draft.riderPhone.replace(/\s+/g, '');
      const { error: riderError } = await supabase.from('riders').upsert({ name: draft.riderName.trim(), phone: riderPhone, is_active: true, updated_at: new Date().toISOString() }, { onConflict: 'phone' });
      setNotice(`${order.order_number || 'Order'} dispatched successfully.${riderError ? ' The rider was used but could not be saved.' : ' The rider has been saved for future orders.'}`);
      setDrafts(current => { const next = { ...current }; delete next[order.id]; return next; }); setSelectedRiders(current => { const next = { ...current }; delete next[order.id]; return next; }); setPicked(current => { const next = { ...current }; delete next[order.id]; return next; }); await load();
    }
    setBusy('');
  }
