...
type DispatchOrder = { id: string; order_number?: string; customer_name?: string; customer_phone?: string; customer_email?: string; delivery_address?: string; total: number; order_items?: Item[] };
...
export default function DispatchPage() {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error: orderError } = await supabase.from('orders').select('id,order_number,customer_name,customer_phone,customer_email,delivery_address,total,order_items(id,product_id,product_name,quantity,unit_price)').eq('status', 'ready_for_dispatch').order('created_at', { ascending: true });
  ...
  async function dispatch(order: DispatchOrder) {
    ...
    else { setNotice(`${order.order_number || 'Order'} dispatched successfully. The customer notification is being processed.`); setDrafts(current => { const next = { ...current }; delete next[order.id]; return next; }); await load(); }
    setBusy('');
  }

  return <main className="mx-auto max-w-6xl p-3 sm:p-6">
    <header className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-black uppercase text-brand-orange">Admin</p><h1 className="flex items-center gap-2 text-3xl font-black text-brand-ink"><Truck/>Dispatch</h1><p className="mt-1 text-sm text-neutral-600">Assign riders and dispatch orders that are ready to leave.</p></div><div className="flex gap-2"><button onClick={() => void load()} className="inline-flex items-center gap-2 rounded-xl border bg-white px-4 py-2 font-bold"><RefreshCw size={17}/>Refresh</button><Link href="/admin" className="rounded-xl border bg-white px-4 py-2 font-bold">Admin</Link><Link href="/admin/orders" className="rounded-xl bg-brand-ink px-4 py-2 font-bold text-white">All orders</Link></div></header>
    ...
    {loading ? <p className="mt-8">Loading dispatch orders…</p> : !orders.length ? <div className="mt-8 rounded-2xl bg-white p-8 text-center shadow-card"><Truck className="mx-auto text-brand-orange"/><h2 className="mt-3 text-xl font-black">No orders ready for dispatch</h2><p className="text-neutral-600">Move an order to Ready for dispatch from the Orders tab first.</p></div> : <div className="mt-6 space-y-5">{orders.map(order => { const draft = drafts[order.id] || emptyDraft; return <article key={order.id} className="overflow-hidden rounded-2xl bg-white shadow-card"><div className="flex flex-wrap items-start justify-between gap-3 bg-brand-soft p-5"><div><p className="text-xs font-black uppercase text-brand-orange">Ready for dispatch</p><h2 className="text-2xl font-black">{order.order_number || order.id}</h2><p>{order.customer_name || 'Guest'} · <a className="font-bold text-brand-orange" href={`tel:${order.customer_phone || ''}`}>{order.customer_phone || 'No phone'}</a></p><p className="mt-1 text-sm text-neutral-600">{order.delivery_address || 'No delivery address'}</p></div><b className="text-xl">{money(order.total)}</b></div><div className="grid gap-5 p-5 lg:grid-cols-[1fr_1.15fr]"><section><h3 className="font-black">Products</h3><div className="mt-3 space-y-2">{(order.order_items || []).map(item => <div key={item.id} className="flex items-center gap-3 rounded-xl border p-2"><img src={images[item.product_id || ''] || PRODUCT_IMAGE_PLACEHOLDER} alt="" className="h-16 w-16 shrink-0 rounded-lg bg-white object-contain p-1"/><div className="min-w-0 flex-1"><b className="block truncate">{item.product_name}</b><small>{item.quantity} × {money(item.unit_price)}</small></div></div>)}</div></section><section className="rounded-2xl border-2 border-brand-orange/30 bg-orange-50 p-4"><h3 className="text-xl font-black">Rider information</h3><div className="mt-3 grid gap-3 sm:grid-cols-2"><label className="text-sm font-bold">Rider name<input value={draft.riderName} onChange={event => update(order.id, 'riderName', event.target.value)} className="mt-1 w-full rounded-xl border bg-white p-3 font-normal" placeholder="Enter rider name"/></label><label className="text-sm font-bold">Rider phone<input value={draft.riderPhone} onChange={event => update(order.id, 'riderPhone', event.target.value)} className="mt-1 w-full rounded-xl border bg-white p-3 font-normal" placeholder="0712 345 678" inputMode="tel"/></label><label className="text-sm font-bold">Delivery note<input value={draft.deliveryNote} onChange={event => update(order.id, 'deliveryNote', event.target.value)} className="mt-1 w-full rounded-xl border bg-white p-3 font-normal" placeholder="Optional instructions"/></label><label className="text-sm font-bold">Tracking URL<input value={draft.trackingUrl} onChange={event => update(order.id, 'trackingUrl', event.target.value)} className="mt-1 w-full rounded-xl border bg-white p-3 font-normal" placeholder="Optional tracking link"/></label></div><button onClick={() => void dispatch(order)} disabled={busy === order.id} className="mt-4 w-full rounded-xl bg-brand-orange px-5 py-3 font-black text-white disabled:opacity-50">{busy === order.id ? 'Dispatching…' : 'Dispatch order'}</button></section></div></article>})}</div>}
  </main>;
}
