...
import { getHomepageSection, getProducts, getTopSellingProductIds } from '@/lib/supabase';
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const description = `Shop ${title.toLowerCase()} from BoozePap with convenient drinks delivery across Nairobi.`;
  return { title, description, alternates: { canonical: `/collections/${slug}` }, openGraph: { title: `${title} | BoozePap`, description, url: `/collections/${slug}`, type: 'website' }, twitter: { card: 'summary', title: `${title} | BoozePap`, description }, robots: builtIn ? { index: true, follow: true } : { index: false, follow: false } };
}
...
export default async function CollectionPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [allProducts, topSellingIds] = await Promise.all([getProducts(), slug === 'top-sellers' ? getTopSellingProductIds(24) : Promise.resolve([])]);
  let title: string, products;
  if (!collection) notFound();
  title = collection.title;
  products = slug === 'top-sellers'
    ? topSellingIds.map(id => allProducts.find(product => product.id === id)).filter((product): product is Product => Boolean(product))
    : allProducts.filter(collection.matches);
  const url = `/collections/${slug}`;
  return <main className="w-full px-4 py-8"><JsonLd data={[{ '@context': 'https://schema.org', '@type': 'CollectionPage', name: title, url: absoluteUrl(url), numberOfItems: products.length }, breadcrumbSchema([{ name: 'Home', url: '/' }, { name: title, url }])]}/><nav aria-label="Breadcrumb" className="mb-4 text-sm text-neutral-600"><a href="/">Home</a> / {title}</nav><div className="rounded-3xl bg-white p-6 shadow-card"><p className="font-bold uppercase tracking-wide text-brand-orange">Shop BoozePap</p><h1 className="text-4xl font-black text-brand-ink">{title}</h1><p className="mt-2 text-neutral-600">Browse every available product in this collection.</p></div>{products.length ? <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-5 xl:grid-cols-6">{products.flatMap(product => { const variants = (product.product_variants || []).filter(variant => variant.is_active !== false); return [<ProductCard key={product.id} p={product}/>, ...variants.slice(1).map(variant => <ProductVariantCard key={variant.id} product={product} variant={variant}/>)]; })}</div> : <p className="mt-6 rounded-2xl bg-white p-6">No active products are currently available in this collection.</p>}</main>;
}
