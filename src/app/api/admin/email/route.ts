...
export async function POST(request: NextRequest) {
  ...
  if (!config.configured) return NextResponse.json({ ...safeEmailStatus(), status: 'not_configured', error: `Missing: ${config.missing.join(', ') || 'EMAIL_PROVIDER'}` }, { status: 503 });
  ...
  const result = await deliverEmail(config.adminEmail, 'BoozePap Email Test Successful', `<div style="font-family:Arial,sans-serif;padding:24px"><h1 style="color:#ff4b18">BoozePap Email Test Successful</h1><p>Your ${config.provider} order-email integration is working correctly.</p></div>`);
  ...
  console.info('[Admin email test] sent', { provider: config.provider, messageId: result.reference });
  return NextResponse.json({ status: 'sent', configured: true, provider: config.provider, messageId: result.reference });
}
