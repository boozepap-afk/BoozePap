...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (productsError) {
      ...
      return safeError('PRODUCT_VERIFICATION_UNAVAILABLE', 'Unable to verify products right now.', 503, requestId);
    }
    ...
    if (variantsError) {
      ...
      return safeError('PRODUCT_VERIFICATION_UNAVAILABLE', 'Unable to verify product sizes right now.', 503, requestId);
    }
    let subtotal = 0;
