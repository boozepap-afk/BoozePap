...
export function HeroCarousel({ banners, categories = [] }: { banners: DbBanner[]; categories?: DbCategory[] }) {
  ...
  return <section className="mx-auto mt-5 grid max-w-[1440px] gap-4 px-4 sm:px-6 lg:grid-cols-[minmax(0,3fr)_minmax(340px,2fr)]">
    ...
    <aside className="rounded-xl border border-neutral-200 bg-white p-5 shadow-sm"><h2 className="text-xl font-black text-brand-ink">What are you after?</h2><p className="mt-1 text-xs text-neutral-500">Pick a shelf, start your basket.</p><div className="mt-4 grid grid-cols-3 gap-2">{featuredCategories.map(category=><Link key={category.id} href={`/category/${category.slug}`} className="group flex min-h-24 flex-col items-center justify-center rounded-xl border border-neutral-200 p-2 text-center transition hover:border-brand-orange hover:bg-brand-soft"><div className="relative h-12 w-12"><SmartImage src={category.image_url || '/boozepap-icon.svg'} alt="" sizes="48px" fit="contain" className="p-1"/></div><span className="mt-1 text-xs font-black text-brand-ink">{category.name}</span></Link>)}</div><Link href="/account" className="mt-4 block rounded-full border-2 border-brand-orange px-4 py-2.5 text-center text-xs font-black text-brand-deep">Sign in for faster checkout →</Link></aside>
  </section>;
}
