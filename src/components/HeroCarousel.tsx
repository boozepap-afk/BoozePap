...
export function HeroCarousel({ banners, categories = [] }: { banners: DbBanner[]; categories?: DbCategory[] }) {
  ...
  return <section className="mx-auto mt-4 max-w-[1280px] px-4 sm:px-6">
    ...
    <div onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onTouchStart={() => setPaused(true)} onTouchEnd={() => setPaused(false)} aria-roledescription="carousel" aria-label="BoozePap promotions" className="relative overflow-hidden rounded-xl bg-neutral-100 shadow-card">
      ...
      <aside className="rounded-xl border border-neutral-200 bg-white p-5 shadow-sm"><h2 className="text-xl font-black text-brand-ink">What are you after?</h2><p className="mt-1 text-xs text-neutral-500">Browse our live drinks categories.</p><div className="mt-4 grid grid-cols-3 gap-2">{featuredCategories.map(category=><Link key={category.id} href={`/category/${category.slug}`} className="group relative min-h-24 overflow-hidden rounded-xl border border-neutral-200 text-center transition hover:border-brand-orange">{category.image_url?<SmartImage src={category.image_url} alt="" sizes="110px" className="transition duration-300 group-hover:scale-105"/>:<span aria-hidden="true" className="grid h-full min-h-24 place-items-center text-3xl">{category.icon||category.name.slice(0,1)}</span>}<span className="absolute inset-x-0 bottom-0 z-10 bg-gradient-to-t from-black/90 to-transparent px-1.5 pb-2 pt-6 text-xs font-black text-white">{category.name}</span></Link>)}</div><Link href="/shop" className="mt-4 block rounded-full border-2 border-brand-orange px-4 py-2.5 text-center text-xs font-black text-brand-deep">Click for more categories →</Link></aside>
    </div>
  </section>;
}
