...
import { readCart, removeUnavailableCartProducts, updateCartQuantity, writeCart } from '@/lib/cart';
...
type DeliveryQuote = { distanceKm: number; deliveryFee: number; source: 'google_routes'|'haversine'; band?: string | null };
...
export function CheckoutClient({ settings, bands }: { settings: CheckoutSettings; bands: DbDeliverySetting[] }) {
  ...
  const [success, setSuccess] = useState<{ orderNumber: string; paymentPending: boolean; message?: string } | null>(null);
  const [coordinates, setCoordinates] = useState<DeliveryLocation | null>(null);
  const [deliveryQuote, setDeliveryQuote] = useState<DeliveryQuote | null>(null), [quoteLoading, setQuoteLoading] = useState(false), [quoteError, setQuoteError] = useState('');
  ...
  useEffect(() => { const refreshCart = () => setItems(readCart()); refreshCart(); window.addEventListener('chupahub-cart-updated', refreshCart); return () => window.removeEventListener('chupahub-cart-updated', refreshCart); }, []);
  useEffect(() => {
    try {
  ...
  const manualBand = useMemo(() => bands.at(-1) || null, [bands]);
  const freeDelivery = productTotal >= 10000;
  const delivery = payment === 'pickup' || freeDelivery ? 0 : manualLocation ? Number(manualBand?.fee || 0) : Number(deliveryQuote?.deliveryFee || 0);
  useEffect(() => {
    if (!coordinates?.verified || manualLocation || payment === 'pickup') { setDeliveryQuote(null); setQuoteError(''); setQuoteLoading(false); return; }
    ...
    const timeout = window.setTimeout(async () => {
      ...
      } catch (cause) { if (!controller.signal.aborted) { setDeliveryQuote(null); setQuoteError(cause instanceof Error ? cause.message : 'Unable to calculate delivery.'); } }
      finally { if (!controller.signal.aborted) setQuoteLoading(false); }
    }, 250);
    return () => { window.clearTimeout(timeout); controller.abort(); };
  }, [coordinates, manualLocation, payment, productTotal]);
